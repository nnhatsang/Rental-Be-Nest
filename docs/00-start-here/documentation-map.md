...
graph TD
    ...
    Architecture --> AsyncRealtime[redis-socket-bullmq-design.md]
...
2. **Redis, BullMQ và Socket.IO theo tính năng**: Đọc [redis-socket-bullmq-design.md](../02-architecture/redis-socket-bullmq-design.md) để chọn đúng công nghệ, thiết kế queue/worker và realtime event theo từng module.
3. **Redis key/cache chi tiết**: Đọc [redis-usage-plan.md](../02-architecture/redis-usage-plan.md) để nắm key pattern, TTL, cache và rate-limit.
4. **Quy chuẩn lập trình**: Đọc [conventions.md](file:///d:/Admin%20Rental/rental-admin-be/docs/04-applications/backend/conventions.md) để biết cách viết Service, Controller, DTO và accent-insensitive search.
